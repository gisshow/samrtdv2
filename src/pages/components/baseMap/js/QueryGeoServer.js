...
class QueryGeoServer {
    constructor(options) {
        ...
        //this.level = Cesium.defaultValue(options.level, "");
    ...
    query(opt) {
        ...
        //叠加PropertyIsLessThanOrEq
        if(opt.levelvalue && opt.type=="Overlay"){
            filter="";
            filter = `<Filter xmlns="http://www.opengis.net/ogc" xmlns:gml="http://www.opengis.net/gml"><And>
                          <PropertyIsEqualTo wildCard="*" singleChar="#" escapeChar="!">
                            <PropertyName>level/level</PropertyName>
                            <Literal>14,15</Literal>
                          </PropertyIsEqualTo>`
        }
