...
class POIList extends Component {
  ...
  showFeature=(item,type)=>{
    ...
    let positions=[];
      ...
      let height=viewer.scene.sampleHeight(Cesium.Cartographic.fromCartesian(position)) || 5;
      let entity=dataSource.entities.add({
        position: Cesium.Cartesian3.fromDegrees(positions[0],positions[1],5),
      // billboard:{
      //   image: encodeURI(iconameurl),//'./config/images/mark/250000.png',
      //   scale: 1, //原始大小的缩放比例
      //   horizontalOrigin: Cesium.HorizontalOrigin.CENTER,
  ...
  onShow=()=>{
    this.setState({isShowSelects:false})
  }
      ...
      <>
        <div className={`${styles.poilist} ${isExpand?'':styles.expand}`}>
          <div className={styles.title}>
            <span style={{marginRight:"10px"}} >公共服务设施列表</span>
            <div className={styles.btn}>
              <span className={styles.options}>{poiType}</span>
              <span className={`iconfont icon_unfold1 ${styles.icon}`} onClick={()=>{
                this.props.onVisible();this.setState({isShowSelects:!isShowSelects})
              }}/>
          ...
          <div className={`${styles.table} ${isExpand?'':styles.expands}`} >
            <ListView
              ref={this.refList}
              source = {list}
              rowHeight={68}
