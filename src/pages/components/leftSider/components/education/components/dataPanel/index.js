...
export default class DataPanel extends Component {
  ...
  render() {
    const { data: { icon, title, name, time,chartData} ,type,hasEye,padding,radius,isShow,hasExpand,height=260} = this.props;
    const {isExpand,statField,range} =this.state;
    // if(chartData["age"]){
    //   this.switchStat("age");
    // }
    return (
      <div className={`${styles.box} ${isExpand===true ? styles.active:''}`} onClick={()=>this.setExpand()}>
        <div className={styles.title}>
          <div className={styles.name}>
            ...
            {
              radius &&  <>
                  <span className={styles.radius} onClick={this.handleData}>缓冲半径</span><Slider min={1} max={2000} defaultValue={100} value={this.props.gap} onChange={(value)=>this.onChangeBoundary(value)}/><span className={styles.radius}>{this.props.gap}m</span>
                </>
            }
          </div>
            ...
            
        {
          isExpand  &&
            ...
            <div className={styles.dashboard}>
              {
                type==="Bar" && chartData[statField] && <Bar data={this.screenOut(chartData[statField].stat)} step={this.getAreaScale(chartData[statField].sum)} type={statField} height={300} padding={padding}/>
              }
              {
            ...
            </div><div className={styles.time}>数据更新 {time}</div></>
        }
          </div>
      </div>
    );
