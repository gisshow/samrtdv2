...
class HouseForBuilding extends Component {
  ...
  componentWillReceiveProps(newPorps){
    const {parcelId} =this.props.House;
  ...
  flyTo = (properties,showPop) => {
    ...
    this.showBuildFeature(properties);
    ...
    this.props.dispatch({
      type: 'House/setBldgNo',
      payload: properties.attributes.bldgNo
    })
  ...
  showBuildFeature=(item)=>{
    this.removeExtraSource();
    let dataSource=new Cesium.CustomDataSource('building-list');
    ...
    this.getLandVecByBuildId(item,dataSource);
  ...
  showEntity= async (feature,showPop)=>{
    ...
    viewer.scene.camera.flyToBoundingSphere(new Cesium.BoundingSphere(polyCenter,height*3), {
      duration: 2,
      complete:()=>{
        mars3d.point.windingPoint.start(viewer, polyCenter);
      }
    });
    // if(showPop){
  ...
  //查询楼所在的地，并加载显示。
  getLandVecByBuildId= async (item,buildSource)=>{
    let geo=JSON.parse(item.location);
    ...
    let buffere = turf.buffer(polygon, 5, { units: 'meters', steps: 64 });
    let holeGeometry=buffere.geometry;
    let result=await getParcelByBuildId({bldgNo:item.attributes.bldgNo});
    if(result.success && result.data){
        this.addLandVecByBuild(result.data,holeGeometry,buildSource);
        
    }else{
      this.setExtraSource([buildSource]);
    }
  }
  addLandVecByBuild =(item,holeGeometry,buildSource)=>{
  ...
  orderList=()=>{
    ...
    buildList && buildList.sort((a,b)=>{
      return isAscOrder ? b.attributes.bldgLdArea-a.attributes.bldgLdArea : a.attributes.bldgLdArea-b.attributes.bldgLdArea;
    })
  ...
  render() {
    // const {buildList,bldgNo,hasHold,detailHeightset,houseHoldModel} = this.props.House;
    ...
    return (
      <>
        <div className={styles.box}>
          ...
          {
            buildList.length>0&&<div className={styles.totalbuild}>{`查找到 ${buildList.length} 条结果`}</div>
          }
              ...
              renderItem = {({item,index,style})=>(
                <div className={(activeId.includes(item.id))? styles.row +' '+styles.active :styles.row} key={index} id={item.id} style={{...style}} onClick={() => this.flyTo(item,true)}>
                  <div className={styles.left}>
                    ...
                    <div className={styles.addr} title={item.attributes.bldaddr}>{item.attributes.bldaddr}</div>
                    <div className={styles.status}>
                      <span>{item.attributes.bldcondName}</span>
                      <span>/</span>
                      <span>{item.attributes.bldstruName}</span>
                    </div>
                  </div>
                  <div className={styles.right}>
                    <span className={styles.state}>{item.attributes.bldgLdArea}m&sup2;</span>
                  </div>
                </div>
              )}
            ></ListView>
