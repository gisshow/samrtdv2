  ...
  getBuildInfo= async(basicId)=>{
    ...
    if(info && info.attributes && info.attributes.bldgNo===basicId){
      ...
      if(info&&info.attributes){
        if(!info.attributes.bldgKey){
          bldgKey=1;  // 永久非永久暂时默认值为1
        }else{
          bldgKey=info.attributes.bldgKey
        }
      }
            ...
            <div className={styles.content}>
              ...
              <div className={styles.group}>
                <div className={styles.item}>
                  ...
                  <span className={styles.value} title={buildingInfo.bldaddr}>{buildingInfo.bldaddr}</span>
                </div>
                <div className={styles.item}>
                  <span className={styles.name}>是否永久建筑:</span>
                  <span className={styles.value} title={buildingInfo.key===1?'永久建筑':buildingInfo.key===2?'非永久建筑':''}>{buildingInfo.key===1?'永久建筑':buildingInfo.key===2?'非永久建筑':''}</span>
                </div>
              </div>
          ...
          </div>
          {
            houseHoldInfo && (
                <div className={styles.footer}>
                  {
                    (houseHoldInfo.length===1) && (<div className={styles.btn} style={{marginRight:'10px'}} onClick={() => this.openHouseHold(houseHoldInfo[0].buildingId)}>查看分层分户</div>)
