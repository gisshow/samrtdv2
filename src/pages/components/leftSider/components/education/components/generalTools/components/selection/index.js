...
class Selection extends Component {
  ...
  clickHandle =(event)=>{
    ...
    Promise.all(promises).then((results)=>{
      ...
      if(land.show && building.show){//查询出2个结果（楼和地）
        // 只显示楼栋
  ...
  showLandFeature=(item,isRelated)=>{
    ...
    dataSource.entities.add({
      polyline:{
          ...
          material:isRelated?Cesium.Color.fromCssColorString("#E1726F"):Cesium.Color.RED.withAlpha(1.0),//Cesium.Color.DIMGRAY.withAlpha(0.8),//Cesium.Color.fromRandom({alpha:1.0})Cesium.Color.DARKGRAY.withAlpha(0.0)
  ...
  componentWillReceiveProps(newPorps){
    ...
    const {houseHoldModel:newHouseHoldModel,isShowMainStat:newIsShowMainStat,isShowHouseBox:newIsShowHouseBox} =newPorps.House;
  ...
  holdIconhandler=(event)=>{
    ...
    if (Cesium.defined(pickedObject) && Cesium.defined(pickedObject.id) && pickedObject.id instanceof Cesium.Entity) {
      ...
      // 判定未分层分户标识
      if(pickedObject.primitive && pickedObject.primitive instanceof Cesium.Billboard && entity.name==="household"){
        var bldgNo=entity.properties.getValue().bldgNo;
        var buildingId=entity.properties.getValue().buildingId;
        var bldgKey = entity.properties.getValue().bldgKey||1; // 永久非永久暂时默认值为1
        bldgNo && this.showFeature({
          type:"building",
          bldgNo:bldgNo,
          bldgKey:bldgKey
        });
  ...
  getBuildInfoByHold= async(bldgNo,bldgKey)=>{
    ...
    // let buildInfo= await getBuildById({basicId :bldgNo,bldgKey:1});
  ...
  setExtraSource=(source)=>{
    ...
    if(extraSource && extraSource.length!==0){
      if(Object.prototype.toString.call(source) =='[object Array]' ){//数组
  ...
  render() {
    const {statType,jdName,detailType,isShowMainStat } = this.props.House;
    const {isRenderTree} = this.state;
    return (
      <div className={`${styles.houseBox} ${(this.state.isShowBg && this.isAllRender())? styles.bgShow : styles.bgHide}`}>
        <>
          {statType.isRenderSubStat && !isShowMainStat && <SubStat data={statType} goHome={()=>this.goHome()} goDetail={()=>this.goDetail()} goTree={()=>this.goTree()}/>}
          {detailType.isRenderDetail && !isShowMainStat && <DetailInfo data={detailType} goStat={() => this.goStat()}/>}
          {(!detailType.isRenderDetail && !isShowMainStat && !statType.isRenderSubStat &&  isRenderTree) && <HouseTree goBack={() => this.goStat()}/>}
        </>
      </div>
    );
  }
