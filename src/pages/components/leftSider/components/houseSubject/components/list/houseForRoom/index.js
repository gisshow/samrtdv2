...
class HouseForRoom extends Component {
  ...
  orderList = () => {
    ...
    roomList &&
      roomList.sort((a, b) => {
        return isAscOrder
          ? b.attributes.houseArea - a.attributes.houseArea
          : a.attributes.houseArea - b.attributes.houseArea;
      });
      ...
      <>
        <div className={styles.box}>
          <div className={styles.title}>
            <span>房屋列表</span>
            <span
              className={`iconfont ${isAscOrder ? 'icon_sequence' : 'icon_reverse'}  ${
                styles.icon
              }`}
              title="按面积排序"
          ...
          {roomList.length > 0 && (
            <div className={styles.totalHouse}>{`查找到 ${roomList.length} 条结果`}</div>
          )}
          <div className={styles.table}>
            <ListView
              ...
              renderItem={({ item, index, style }) => (
                <div
                  className={item.id === activeId ? styles.row + ' ' + styles.active : styles.row}
                  key={index}
                  id={item.id}
                  style={{ ...style }}
                ...
                >
                  <div className={styles.left}>
                    <div className={styles.name} title={item.attributes.roomName}>
                      {item.attributes.roomName}
                    </div>
                    <div className={styles.addr} title={item.attributes.certAddr}>
                      {item.attributes.certAddr}
                    </div>
