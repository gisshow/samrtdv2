...
class FilterItem extends Component {
        ...
        this.handler.setInputAction((movement) => {
            ...
            if(pickedObject && pickedObject.id){
                ...
                if(this.QuPolygonHash[name]){
                    this.QuPolygonHash[name].polygon.material = Cesium.Color.BLANCHEDALMOND.withAlpha(0.3)
                ...
                if(this.JdPolygonHash[name]){
                    this.JdPolygonHash[name].polygon.material = Cesium.Color.BLANCHEDALMOND.withAlpha(0.3)
                    this.hightLightName=name;
                    // console.log(2,this.hightLightName);
                }
            }else{
    ...
    hide=()=>{
        ...
        // if(activeNames[0] && this.streetEntityHash[activeNames[0]]){
        //     this.streetEntityHash[activeNames[0]].show=false;
        // }
        // if(activeNames[1] && streetEntityHash[activeNames[1]]){
        //     streetEntityHash[activeNames[1]].show=false;
        // }
    }
    hideDistinct=()=>{
        this.distinctEntity.map((item,index)=>{
    ...
    flyTo=(bbox,isSelfHand)=>{
        ...
        this.props.getRegionLabel(regionLabel,paramDataObjId);
    ...
    topNodeClick = (item, isSelfHand) => {
        ...
        if (item.id === TEXT_ALL_VALUES[0].id || menuActiveId === item.name) {
            ...
            this.hideDistinct();
            console.log(TEXT_ALL_VALUES[1].id)
    ...
    secondNodeClick= (item, isSelfHand) => {
        ...
        // if(this.streetEntityHash[activeNames[0]]){
            if(activeNames[0]!==item.belongDistrict && item.belongDistrict){
                if(this.streetEntityHash[activeNames[0]]){
            ...
            if(item.belongDistrict && this.streetEntityHash[item.belongDistrict]){
