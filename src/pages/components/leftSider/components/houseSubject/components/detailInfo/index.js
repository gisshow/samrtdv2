  ...
  renderDetail = () => {
    ...
    switch (type) {
        ...
        return (
          bldgNo && (
            <BuildingInfo
              basicId={bldgNo}
              info={info}
              goPopulationStat={this.goPopulationStat}
              goPOIStat={this.goPOIStat.bind(this)}
              info={info}
            />
          )
        );
      case 'room':
        return (
          houseId && (
            <RoomInfo houseId={houseId} goPOIStat={this.goPOIStat.bind(this)} info={info} />
          )
        );
      case 'parcel':
      case 'land':
        ...
        return (
          info && (
            <ParcelMoreDetail
              show={isShowMore}
              info={info}
              close={() => this.setState({ isShowMore: false })}
            />
          )
        );
      case 'building':
      ...
      <>
        <div className={styles.box}>
          <ModuleTitle title={`${type === 'other' ? title : `${title}详情`}`}>
            {type !== 'other' && (
              <div className={styles.btn} onClick={() => this.goMore()}>
                <span>更多</span>
        ...
        <div className={styles.detail}>
          ...
          <div className={styles.detailList}>{this.renderList()}</div>
          <div className={styles.otherList}>
            {isShowMore && this.renderMore()}
            {activeKey === 'mainStat' && (
              <div className={styles.footer} onClick={() => this.goStat()}>
                <span>返回统计信息</span>
              </div>
            )}
            {activeKey === 'query' && (
