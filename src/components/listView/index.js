...
export default class List extends React.Component {
  ...
  setScrollPosition = event => {
    ...
    this.getWrapper().scrollTo(0, event.target.scrollTop)
  }
  ...
  showItem = (index) => {
    let res = index * this.props.rowHeight;

    this.setScrollPosition({target:{scrollTop:res}});
  }


  componentWillReceiveProps(nextProps){
    if(!this.props)return;
    if((nextProps.scrollToId !== this.props.scrollToId) && nextProps.scrollToId ){

      let index = this.props.source.map(item=>item.id).indexOf(nextProps.scrollToId)
      if(index==-1)return;
      this.showItem(index)
    }
  }
  ...
  renderList = () => (
    <div
      ...
      <div className={styles.scrollBox} style={style.listWrapper} ref={c => (this.listWrapper = c)}>
