...
var tableWork = {
    ...
    initEvent: function () {
        ...
        $("#btnFlyStart").click(function (e) {
            ...
            //隐藏面板
            if(id==""){
               if(thisWidget.iframeleft!=0){
                $(window)[0].frameElement.parentElement.parentElement.style.left=$(window)[0].frameElement.parentElement.parentElement.offsetLeft+450;   
               }             
                plotEdit.linedataList()          
            }
        });
            ...
            },
            {
                ...
                events: {
                    ...
                    'click .edit': function (e, value, row, index) {
                       ...
                       //thisWidget.startEditingById(row.id);
                    },
                },
                formatter: function (value, row, index) {
                    return [
                        '<a class="fly"  href="javascript:void(0)" title="飞行"><i class="fa fa-send-o"></i></a>&nbsp;&nbsp;',
                        '<a class="edit" id="'+row.id+'edit" href="javascript:void(0)" title="编辑"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;',
                        '<a class="remove" id="'+row.id+'remove"  href="javascript:void(0)" title="删除"><i class="fa fa-trash"></i></a>'
                    ].join('');
                }
            }
            ],
    ...
    startedit:function(id){
         $("#btnFlyStart").html("开启漫游")
         $("#"+id+"edit").click();
    },
    startdarwline:function(){
        $("#btn_Add_line").click();
    },
    removeflyline:function(id){
        $("#"+id+"remove").click();
        return plotEdit.linedataList();
    }
};
...
var plotEdit = {
    ...
    linedataList:function(){
       if(thisWidget.arrFlyTable){
           return thisWidget.arrFlyTable;
       }
    },
        ...
        if (!latlngs || latlngs.length == 0) {
            ...
            for (var idx = 0; idx < latlngs.length; idx++) {
                ...
                if (cfg.maxCount == null || latlngs.length < cfg.maxCount)
                    addthml = '<i class="fa  fa-plus latlng-install" title="插入点" data-index="' + idx + '" ></i>&nbsp;&nbsp;';

                // 
                inHtml += '<div><div class="open"><i class="tree_icon">-</i>第' + (idx + 1) + '点 <label style="width:158px;">&nbsp;</label>    ' + addthml + delhtml + ' </div><div class="mp_attr"><table>' +
