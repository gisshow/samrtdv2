...
function initWidgetView(_thisWidget) {
    ...
    var followedZ = attr.camera.followedZ || 100
    ...
    //继续
    $("#btnFlyproceed").click(function (event) {
        var cameraType = $("#cameraType").attr('data-value');
        var followedX = Number($("#followedX").val());
        var followedZ = Number($("#followedZ").val());
        $("#btnFlyproceed").css("display","none")
        $("#btnFlypause").css("display","initial")
        thisWidget.RoamLineproceed({
            camera: {
                type: cameraType,
                followedX: followedX,
                followedZ: followedZ
    ...
    //停止
    $("#btnFlypause").click(function (event) {
             
        $("#btnFlypause").css("display","none")
        $("#btnFlyproceed").css("display","initial")
        thisWidget.RoamLinepause({
            camera: {
                type: "",
                followedX: 200,
                followedZ: 100
